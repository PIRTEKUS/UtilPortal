...
- **Isolated Python Execution:** Execute custom Python modules safely in automatically generated, isolated virtual environments (`venv`). Dependencies are dynamically installed via `requirements.txt` or auto-generated by analyzing your script's imports.
- **Real-Time Streaming:** Watch the console output of Python scripts stream to the browser in real-time via Server-Sent Events (SSE).
